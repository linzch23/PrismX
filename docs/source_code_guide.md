...
PrismX 是一个本地运行的长期认知型 Agent 框架，核心目标是把智能体的运行上下文从“线性聊天历史”升级为“Tree-Guided Memory（TGM，树引导记忆）”。当前代码已经具备命令行 Agent、Web 工作台、工具调用、MCP 工具接入、TreeSession 会话树、Active Branch Context、Working Set、上下文压缩、结构化长期记忆、Wiki-style Knowledge Base、本地语义索引、运行时召回、一次性子代理和持久 TeamMate 等能力。
...
| 目录/文件 | 作用 |
|---|---|
| `src/prismx/` | Agent Runtime、会话树、记忆系统、工具系统、MCP、多智能体等核心代码。 |
...
| 项 | 内容 |
|---|---|
| 打包命令 | `prismx = "prismx.cli:main"`，定义在 `pyproject.toml` |
| 入口文件 | `src/prismx/cli.py` |
| 核心函数 | `main()` |
| 应用装配 | `AgentApp`，定义在 `src/prismx/loop.py` |

CLI 启动流程：

```text
uv run prismx
→ src/prismx/cli.py::main()
→ 创建 AgentApp
...
| 项 | 内容 |
|---|---|
| 打包命令 | `prismx-web = "prismx.server:main"`，定义在 `pyproject.toml` |
| 入口文件 | `src/prismx/server.py` |
| 核心函数 | `main()` |
| Web 适配 | `src/prismx/web_adapter.py` |

Web 工作台复用 `AgentApp` 应用层，通过 HTTP/SSE 暴露聊天流、会话树、上下文、工具、MCP 和 team 状态。

### `AgentApp` 初始化流程

`src/prismx/loop.py::AgentApp.__init__()` 负责装配核心模块：
...
| 功能 | 代码路径 | 核心函数/类 | 作用说明 |
|---|---|---|---|
| 用户输入接收 | `src/prismx/cli.py` | `main()` | CLI 中读取用户输入，识别 Slash Command 或转给 `AgentApp.ask()`。 |
| 应用装配 | `src/prismx/loop.py` | `AgentApp` | 统一管理模型、工具、会话树、记忆、MCP、team 和上下文构建。 |
| 普通对话入口 | `src/prismx/loop.py` | `AgentApp.ask()` | 写入用户消息、构建 Runtime Recall 与 Working Set、调用 Runner。 |
| Working Set 构建 | `src/prismx/working_set.py` | `WorkingSetBuilder.build()` | 从 active branch、todo、最近工具结果、episode 摘要和召回结果生成工作集。 |
| System Prompt 构造 | `src/prismx/context.py` | `ContextBuilder.build()` | 把 workspace、skills、memory、user profile、runtime context 渲染进 prompt。 |
| LLM 调用 | `src/prismx/runner.py` | `AgentRunner.step()` | 调用 `client.create_message()` 或 `client.stream_message()`。 |
| Tool Call 解析 | `src/prismx/runner.py` | `AgentRunner.step()` | 从模型返回的 `message.content` 中筛出 `tool_use` block。 |
| Tool 执行 | `src/prismx/runner.py` | `_execute_tool_blocks()` | 通过 `ToolRegistry.execute()` 执行工具，支持安全工具并行。 |
| Tool Result 回注 | `src/prismx/runner.py`、`src/prismx/loop.py` | `on_tool_result` 回调 | 工具结果作为 user message 继续喂给模型，并写入 TreeSession。 |
| 多轮循环 | `src/prismx/runner.py` | `AgentRunner.step()` | 直到模型 `stop_reason != "tool_use"` 或达到 `max_turns`。 |
| 中间状态保存 | `src/prismx/tree_session.py` | `append_message()`、`append_tool_call()`、`append_tool_result()` | 用户消息、assistant 消息、工具调用、工具结果写入 JSONL 会话树。 |
...
Slash Command 当前集中写在 `src/prismx/cli.py::main()` 的输入循环中。它不是独立 parser，而是通过一组 `if user_input ...` 判断直接分发。

| 命令 | 代码位置 | 核心函数 | 功能说明 |
|---|---|---|---|
| `/help` | `src/prismx/cli.py` | `HELP`、`main()` | 打印命令帮助。 |
| `/tools` | `src/prismx/cli.py` | `app.registry.names()` | 列出当前注册工具。 |
| `/todos` | `src/prismx/cli.py` | `app.todos.render()` | 显示当前 todo 状态。 |
| `/memory` | `src/prismx/cli.py` | `app.memory.render_memory()` | 显示结构化长期记忆和 legacy MEMORY。 |
| `/context` | `src/prismx/cli.py` | `app.working_context_debug()`、`app.memory.list_context()` | 显示 Working Set 调试信息和最近 ContextObject。 |
| `/mcp` | `src/prismx/cli.py` | `app.mcp.report()` | 显示 MCP server 和工具状态。 |
| `/compact` | `src/prismx/cli.py` | `app.compact_now()` | 压缩当前 active branch，并触发记忆提交。 |
| `/team` | `src/prismx/cli.py` | `app.team.list_all()` | 查看持久 TeamMate 状态。 |
| `/inbox` | `src/prismx/cli.py` | `app.team_bus.read_inbox("lead")` | 读取并清空 lead inbox。 |
| `/tree` | `src/prismx/cli.py` | `app.tree_view()` | 渲染当前会话树。 |
| `/jump ID` | `src/prismx/cli.py` | `app.jump_to_entry()` | 把 active leaf 切换到历史节点。 |
| `/fork ID` | `src/prismx/cli.py` | `app.fork_from_entry()` | 选择分叉点，下一条输入形成兄弟分支。 |
| `/clone` | `src/prismx/cli.py` | `app.clone_active_branch()` | 克隆当前 active branch 到新 session。 |
| `/label ID LABEL` | `src/prismx/cli.py` | `app.label_entry()` | 给会话树节点追加标签。 |
| `/exit` | `src/prismx/cli.py` | `return` | 退出 CLI。 |

补充命令：代码中还支持 `/skills` 和 `/skill NAME`，用于列出和加载技能，但它们不在用户要求的最小命令清单内。

## 6. TreeSession 与会话树源码说明

TreeSession 的核心实现在 `src/prismx/tree_session.py`。
...
PrismX 当前不是直接把全量历史拼给模型，而是使用 active branch 加运行时召回构造当前轮次上下文。
...
| Working Context | `WorkingSet`，定义在 `src/prismx/working_set.py`。 |
| Runtime Recall 注入内容 | `RuntimeContextBuilder.build()`，定义在 `src/prismx/context.py`。 |
| 最终 Working Set | `WorkingSetBuilder.build()` 产出后通过 `WorkingSet.render()` 注入 system prompt。 |

### 构建逻辑

`src/prismx/loop.py::AgentApp.ask()` 的顺序：
...
`memory/` 由 `src/prismx/memory.py::MemoryStore`、`TreeMemoryStore` 和长期知识索引初始化。它不是 TreeSession 的唯一事实源；原始任务经历的主事实源是 `sessiontrees/*.jsonl`。`memory/` 主要负责 Tree Memory、长期知识、关系索引和召回索引。
...
定义在 `src/prismx/tree_session.py`：
...
`src/prismx/knowledge.py::KnowledgeObject`：
...
`src/prismx/context.py::RuntimeContextBuilder.build()`。
...
PrismX 支持两类多智能体机制：一次性子代理 `dispatch_subagent` 和持久队友 `spawn_teammate`。

### dispatch_subagent

| 项 | 内容 |
|---|---|
| 工具类 | `src/prismx/tools/dispatch.py::DispatchSubagentTool` |
| 子代理注册 | `src/prismx/subagents/registry.py::SubagentRegistry` |
| 子代理规格 | `src/prismx/subagents/spec.py::SubagentSpec` |
...
| 项 | 内容 |
|---|---|
| 工具类 | `src/prismx/tools/team.py::SpawnTeammateTool` |
| 管理器 | `src/prismx/team.py::TeammateManager` |
| 消息总线 | `src/prismx/team.py::MessageBus` |
| 注册位置 | `AgentApp._build_registry()` |
...
| 工具封装 | `src/prismx/tools/team.py::SendMessageTool`、`ReadInboxTool`、`BroadcastTool` |
...
读取函数：`src/prismx/mcp_bridge.py::MCPClientManager._load_specs()`。
...
`src/prismx/tools/registry.py::ToolRegistry` 负责：
...
```text
User Input
→ src/prismx/cli.py 判断 Slash Command 或普通输入
→ AgentApp.ask()
...
1. 先看 `pyproject.toml`，确认命令入口 `prismx` 和 `prismx-web`。
2. 看 `src/prismx/cli.py`，理解 CLI 如何接收输入和分发 Slash Command。
3. 看 `src/prismx/loop.py`，理解 `AgentApp` 如何装配模型、工具、会话树、记忆和 MCP。
4. 看 `src/prismx/runner.py`，理解 LLM 调用、工具调用、多轮循环和工具结果回注。
5. 看 `src/prismx/tree_session.py`，理解 JSONL 会话树、active branch、jump/fork/clone、compaction。
6. 看 `src/prismx/working_set.py`、`src/prismx/context.py`、`src/prismx/context_backend.py`，理解 Working Set 和 Runtime Recall。
7. 看 `src/prismx/memory.py`、`src/prismx/contextfs.py`、`src/prismx/knowledge.py`、`src/prismx/memory_graph.py`，理解长期记忆、Wiki 知识库、语义索引和关系图。
8. 看 `src/prismx/tools/`，理解内置工具的接口、参数校验和注册。
9. 看 `src/prismx/mcp_bridge.py`，理解 MCP 工具如何接入。
10. 看 `src/prismx/team.py`、`src/prismx/tools/dispatch.py`、`src/prismx/tools/team.py`，理解多智能体。
11. 最后看 `tests/`，用测试反推关键行为和边界。

## 18. 总结

当前 PrismX 代码已经形成一个可运行的 TGM Agent Runtime：`AgentApp` 负责装配，`AgentRunner` 负责模型和工具循环，`TreeSessionManager` 负责树状任务经历和 active branch，上下文由 `WorkingSetBuilder` 和 `RuntimeContextBuilder` 动态构建，长期知识通过 `MemoryStore`、`ContextFS`、`WikiKnowledgeBase`、`LocalSemanticVectorIndex` 和 `MemoryGraph` 沉淀与召回。
